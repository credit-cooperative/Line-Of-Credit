...
// icETH address 0x7C07F7aBe10CE8e33DC6C5aD68FE033085256A84
// icETH manager 0xb97F5a34696adf30db822612379235c3c53B714A
// icETH methodologist 0x6904110f17feD2162a11B5FA66B188d801443Ea4
// icETH operator 0x6904110f17feD2162a11B5FA66B188d801443Ea4
// icETH controller 0xa4c8d221d8BB851f83aadd0223a8900A6921A349
// icETH feesplitExtension 0x1B7d68134ef476B7795b22a4d76F82c220c54E74
// icETH feeSplitExtension Owner 0x6904110f17feD2162a11B5FA66B188d801443Ea4
...
contract IndexRe7Sim is Test {
   ...
   // dsETH Addresses
    ...
    address constant dsETHManagerAddress = 0xBB6134ba82192E0ab23De846f1Cae7aa9Ae383d5; // Manager of the dsETH product
    ...
    address constant icETHFeeSplitExtension = 0x1B7d68134ef476B7795b22a4d76F82c220c54E74;
    address constant icETHManagerAddress = 0xb97F5a34696adf30db822612379235c3c53B714A;
    address constant icETHOperator = 0x6904110f17feD2162a11B5FA66B188d801443Ea4;
    ...
    function setUp() public {
        ...
        dsETHManager = IManager(dsETHManagerAddress);

        icETH = icETHFeeSplitExtension(icETHFeeSplitExtension);
        icETHManager = IManager(icETHManagerAddress);
    ...
    function test_index_re7_simulation() public {
        ...
        _initSpigot(100, 0x000000, newOwnerFunc, dsETHManagerAddress);
        _initSpigot(50, 0x000000, newOwnerFunc, icETHManagerAddress);
        ...
        dsETHManager.setMethodologist(address(securedLine.spigot()));

        // dsETH.updateOperatorFeeRecipient(address(securedLine.spigot()));

        assertEq(address(securedLine.spigot()), dsETHManager.methodologist());
        ...
        // Index Coop sets Spigot as methodologist to Spigot

        vm.startPrank(icETHOperator);
        emit log_named_string("\n \u2713 icETH Operator Sets Spigot as OperatorFeeRecipient", "");


        emit log_named_string("\n \u2713 dsETH Operator Sets Methodologist Fee Split to 100%", "");
        icETH.updateFeeSplit(0); // Setting operator fee split to 0% with automatically makes methodologist 100%
        icETH.updateFeeSplit(0); // call twice because requires mutual upgrade of both methodologist and operator

        emit log_named_uint("\n - Operator Fee Split: ", icETH.operatorFeeSplit());
        assertEq(0, icETH.operatorFeeSplit());

        emit log_named_string("\n \u2713 icETH Operator Sets Spigot as Methodologist", "");
        icETHManager.setMethodologist(address(securedLine.spigot()));

        // dsETH.updateOperatorFeeRecipient(address(securedLine.spigot()));

        assertEq(address(securedLine.spigot()), icETHManager.methodologist());
        //assertEq(address(securedLine.spigot()), dsETH.operatorFeeRecipient());
        ...
        //TODO: Simulate CC whitelisting setManager, and then IndexCoop changes manager via operate function on Spigot
        //TODO: Simulate 0x trades to make sure claimAndTrade function works

        // fast forward 3 months
        emit log_named_string("\n<---------- Fast Forward 89 Days --------------------> ", "");
        vm.warp(block.timestamp + (ttl - 1 days));

        // Lender address calls accrueFeesAndDistribute function on dsETH and icETH Fee Split Extension contract
        ...
        uint256 dsETHBalanceOfSpigotAfterClaim = IERC20(dsETHToken).balanceOf(address(securedLine.spigot()));
        emit log_named_uint("- Balance of Spigot after Fee Distribution", dsETHBalanceOfSpigotAfterClaim);

        icETH.accrueFeesAndDistribute();
        uint256 icETHBalanceOfSpigotAfterClaim = IERC20(icETHToken).balanceOf(address(securedLine.spigot()));
        emit log_named_uint("- Balance of Spigot after Fee Distribution", icETHBalanceOfSpigotAfterClaim);

        vm.stopPrank();

        // claim revenue
        emit log_named_string("\n \u2713 [Borrower/Lender/Arbiter] Calls the Spigot Claim Function", "");
        _claimRevenueOnBehalfOfSpigot(0x000000, icETHManagerAddress, icETHToken);
        _claimRevenueOnBehalfOfSpigot(0x000000, dsETHManagerAddress, dsETHToken);
        ...
        */
        ...
        // spigot.removeSpigot(dsETHManagerAddress);
        ...
        address whoIsMethodologist = dsETHManager.methodologist();
        emit log_named_address("- The Methodologist of dsETH is ", whoIsMethodologist);
        emit log_named_address("- Spigot address is ", address(securedLine.spigot()));

        emit log_named_string("\n \u2713 Borrower Removes Spigot", "");
        spigot.removeSpigot(dsETHManagerAddress);
        whoIsMethodologist = dsETHManager.methodologist();
        ...
        dsETHManager.setMethodologist(dsETHOperator);
        vm.stopPrank();
        ...
        // address whoIsOperator = manager.operator();
        whoIsMethodologist = dsETHManager.methodologist();
        emit log_named_address("- The Operator of dsETH is ", address(dsETHManager.operator()));
        emit log_named_address("- The Methodologist of dsETH is ", whoIsMethodologist);
        // emit log_named_address("- Spigot Oaddress is ", dsETHOperator);
        assertEq(dsETHOperator, dsETHManager.methodologist());
    ...
    function _claimRevenueOnBehalfOfSpigot(bytes4 claimFunc, address revContract, address revToken) internal {

        bytes memory data = abi.encodeWithSelector(claimFunc);
        (uint8 _split, bytes4 _claim, bytes4 _transfer) = spigot.getSetting(revContract);
        emit log_named_bytes4("func being called", bytes4(data));
        emit log_named_bytes4("stored value", _claim);
        uint256 claimed = spigot.claimRevenue(revContract, revToken, data);
    ...
    function _initSpigot(
        ...
        require(
            spigotedLine.addSpigot(revenueContract, settings),
    ...
    function test_arbiter_adds_revenue_contract_to_spigot() public {
        ...
        (uint8 split2, bytes4 claimFunc2, bytes4 transferFunc2) = spigot2.getSetting(dsETHManagerAddress);
