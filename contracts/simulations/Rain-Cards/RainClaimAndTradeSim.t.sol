...
contract RainRe7Sim is Test {
    ...
    address rainController = 0xE5D3d7da4b24bc9D2FDA0e206680CD8A00C0FeBD;
    ...
    function test_claim_and_trade_and_repay() public {
        ...
        uint256 balanceOfLineBefore = IERC20(creditToken).balanceOf(Rainline);

        bytes memory claimFuncData = abi.encodeWithSelector(claimFunc);


        ISpigot(spigot).claimRevenue(rainController, claimToken, claimFuncData);
        
        vm.prank(servicer);
        spigotedLine.claimAndRepay(claimToken, tradeData);

        uint256 balanceOfLineAfter = IERC20(creditToken).balanceOf(Rainline);
